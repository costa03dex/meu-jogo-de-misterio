...
function gerenciarMusicaFundo() {
    if (tempoRestante <= 30 && tempoRestante > 0 && estadoJogo !== "INTRO" && estadoJogo !== "FIM" && estadoJogo !== "FIM_TEMPO" && estadoJogo !== "EXPLICACAO_FINAL") {
...
function prepararExplicacao() {
    if (suspeitoSelecionado === 0 && pistasColetadas >= totalPistas) {
        textosExplicacao = [
            "Você apresentou todas as provas irrefutáveis contra o Rival.",
            "A luva de luxo esquecida na janela de Tião, o lenço sujo na cena do crime, e os relatos sobre sua fuga e seu plano macabro...",
            "Sem saída, o Rival desabou. Ele confessou que roubou o relógio para incriminar Tião e tomar suas terras a preço de banana.",
            "O relógio foi recuperado e Tião foi declarado completamente inocente perante a cidade!"
        ];
    } else if (suspeitoSelecionado === 0 && pistasColetadas < totalPistas) {
        textosExplicacao = [
            "Você apontou o Rival como o grande culpado pelo crime.",
            "No entanto... a acusação foi baseada apenas em suposições. Faltavam provas concretas conectando ele à cena do crime.",
            "Com advogados caros, o Rival desqualificou rapidamente a sua denúncia no tribunal.",
            "Ele saiu rindo pela porta da frente, enquanto o verdadeiro paradeiro do relógio continuou sendo um mistério..."
        ];
    } else if (suspeitoSelecionado === 2) {
        textosExplicacao = [
            "Você virou-se para o Padre e o acusou perante toda a cidade.",
            "Um murmúrio de choque tomou conta do local. Como um homem de fé faria tal coisa?",
            "Abalado e incapaz de mentir, o Padre caiu de joelhos aos prantos.",
            "Ele confessou ter roubado a relíquia em um momento de desespero para pagar dívidas obscuras da paróquia que estava afundando..."
        ];
    } else {
        textosExplicacao = [
            `Você acusou firmemente ${suspeitosNomes[suspeitoSelecionado]} pelo roubo.`,
            "A polícia acreditou no seu instinto de detetive e prendeu o suspeito imediatamente.",
            "Semanas depois, porém, o relógio de ouro apareceu sendo vendido em um leilão clandestino muito longe dali.",
            "Você percebeu, tarde demais, que mandou um inocente para trás das grades enquanto o verdadeiro ladrão fugia ileso no trem..."
        ];
    }
}
...
function acionarAcao(tecla) {
    ...
    if (cadernoAberto) {
        if (tecla === " ") { somMenuOpcoes.play().catch(()=>{}); cadernoAberto = false; }
        return; 
    }
    ...
    if (estadoJogo === "ACUSACAO") {
        ...
        if (tecla === " ") { 
            somClickDialogo.play().catch(()=>{}); 
            estadoJogo = "CONFIRMACAO"; // Vai para a tela de confirmação
        }
        return;
    }

    // --- NOVA TELA: CONFIRMAÇÃO ---
    if (estadoJogo === "CONFIRMACAO") {
        if (tecla === "1") { 
            somPista.play().catch(()=>{}); 
            prepararExplicacao();
            gerenciarMusicaFundo(); // Pausa música tensa
            estadoJogo = "EXPLICACAO_FINAL";
            charIndex = 0;
            explicacaoFase = 0;
        }
        if (tecla === "2") {
            somMenuOpcoes.play().catch(()=>{}); 
            estadoJogo = "ACUSACAO"; // Cancela e volta a escolher
        }
        return;
    }

    // --- NOVA TELA: EXPLICAÇÃO DO CASO ---
    if (estadoJogo === "EXPLICACAO_FINAL") {
        if (tecla === " ") {
            somClickDialogo.play().catch(()=>{});
            if (charIndex < textosExplicacao[explicacaoFase].length) {
                charIndex = textosExplicacao[explicacaoFase].length;
            } else {
                explicacaoFase++; charIndex = 0;
                if (explicacaoFase >= textosExplicacao.length) estadoJogo = "FIM";
            }
        }
        return;
    }
    ...
    if (tecla === " " && estadoJogo === "EXPLORANDO") {
        ...
        if (pertoNPC) {
            somClickDialogo.play().catch(()=>{}); 
            npcFoco = pertoNPC; textoResposta = ""; charIndex = 0; estadoJogo = "DIALOGO";
        } else if (pertoItem) {
            somPista.play().catch(()=>{}); 
            pertoItem.coletado = true; pistasColetadas++; anotacoes.push(pertoItem.textoPista); 
            estadoJogo = "DIALOGO"; npcFoco = { nome: "NOVA ANOTAÇÃO", tipo: "sistema" };
            textoResposta = `Você anotou uma pista no caderno! Aperte C para ler.`; charIndex = 0;
        }
    } 
    else if (tecla === " " && estadoJogo === "DIALOGO" && textoResposta !== "") {
        if (charIndex < textoResposta.length) { charIndex = textoResposta.length; } 
        else { somClickDialogo.play().catch(()=>{}); estadoJogo = "EXPLORANDO"; npcFoco = null; }
    }
        ...
        if (tecla === "1") {
            ...
            if (npcFoco.tipo === "rival") { estadoJogo = "RIVAL_DIALOGO"; falaRival = 0; charIndex = 0; } 
                ...
                if (npcFoco.daPista1 && !npcFoco.pistaColetada) { 
                    somPista.play().catch(()=>{}); pistasColetadas++; npcFoco.pistaColetada = true; if (npcFoco.textoPista1) anotacoes.push(npcFoco.textoPista1);
                }
            }
        }
        if (tecla === "2") {
            ...
            if (npcFoco.daPista2 && !npcFoco.pistaColetada) { 
                somPista.play().catch(()=>{}); pistasColetadas++; npcFoco.pistaColetada = true; if (npcFoco.textoPista2) anotacoes.push(npcFoco.textoPista2);
            }
        }
    }
}
...
function criarBotaoMobile(txt, props, tecla) {
    ...
    btn.style.alignItems = "center"; btn.style.justifyContent = "center"; btn.style.fontSize = props.fontSize || "26px";
    btn.style.fontWeight = "bold"; btn.style.userSelect = "none"; btn.style.zIndex = "100"; btn.style.touchAction = "none"; 
...
function draw() {
    ...
    if (estadoJogo === "EXPLICACAO_FINAL") {
        ...
        if (charIndex < txtOriginal.length) charIndex += textSpeed;
        wrapText(ctx, txtOriginal.substring(0, Math.floor(charIndex)), canvas.width / 2, canvas.height / 2 - 30, 1000, 45);
    ...
    if (estadoJogo === "ACUSACAO" || estadoJogo === "CONFIRMACAO") {
        ...
        if (inteImg.complete && inteImg.naturalWidth > 0) {
            ...
            ctx.fillText("Use SETAS para escolher e A/ESPAÇO para acusar", canvas.width / 2, startY + imgH + 110);
        }

        // Desenha a caixa de Confirmação por cima
        if (estadoJogo === "CONFIRMACAO") {
            ctx.fillStyle = "rgba(0, 0, 0, 0.9)"; ctx.fillRect(0, 0, canvas.width, canvas.height); // escurece o fundo
            
            let cx = canvas.width / 2; let cy = canvas.height / 2;
            ctx.fillStyle = "#1e293b"; ctx.fillRect(cx - 300, cy - 150, 600, 300);
            ctx.strokeStyle = "#ffe600"; ctx.lineWidth = 4; ctx.strokeRect(cx - 300, cy - 150, 600, 300);
            
            ctx.fillStyle = "white"; ctx.font = "bold 32px Arial";
            ctx.fillText(`Você tem certeza que quer acusar ${suspeitosNomes[suspeitoSelecionado]}?`, cx, cy - 60);
            
            let corProvas = pistasColetadas === totalPistas ? "#64ffda" : "#ff4444";
            ctx.fillStyle = corProvas; ctx.font = "bold 26px Arial";
            ctx.fillText(`Provas encontradas: ${pistasColetadas}/${totalPistas}`, cx, cy);
            
            ctx.fillStyle = "white"; ctx.font = "22px Arial";
            ctx.fillText("[ 1 ] SIM, Tenho certeza", cx, cy + 70);
            ctx.fillText("[ 2 ] NÃO, Voltar para seleção", cx, cy + 110);
        }

        ctx.textAlign = "left"; requestAnimationFrame(gameLoop); return;
    }

    if (mapaImg.complete && mapaImg.naturalWidth > 0) { ctx.drawImage(mapaImg, 0, 0, canvas.width, canvas.height); } 
    else { ctx.fillStyle = "#333"; ctx.fillRect(0, 0, canvas.width, canvas.height); }

    for (let item of itensCenario) {
        if (!item.coletado) {
            ctx.fillStyle = "gold"; ctx.beginPath(); ctx.arc(item.x + item.w/2, item.y + item.h/2, 20, 0, Math.PI * 2); ctx.fill();
    ...
    let raioLanterna = 190; let centroX = detetive.x + detetive.w / 2; let centroY = detetive.y + detetive.h / 2;
    let gradiente = lCtx.createRadialGradient(centroX, centroY, 50, centroX, centroY, raioLanterna);
    gradiente.addColorStop(0, 'rgba(0,0,0,1)'); gradiente.addColorStop(1, 'rgba(0,0,0,0)');
    lCtx.fillStyle = gradiente; lCtx.beginPath(); lCtx.arc(centroX, centroY, raioLanterna, 0, Math.PI * 2); lCtx.fill();
    ...
    if (tempoRestante <= 30 && (estadoJogo === "EXPLORANDO" || estadoJogo === "DIALOGO")) {
        ...
        ctx.fillStyle = `rgba(255, 0, 0, ${alpha})`; ctx.fillRect(0, 0, canvas.width, canvas.height);
    }
    ...
    if (avisoSistema !== "" && timerAviso > 0 && !cadernoAberto) {
        ctx.fillStyle = "rgba(255, 50, 50, 0.9)"; ctx.fillRect(canvas.width / 2 - 350, 80, 700, 50);
        ...
        ctx.fillText("⚠️ " + avisoSistema, canvas.width / 2, 113); ctx.textAlign = "left"; 
    }
    ...
    ctx.fillStyle = "#64ffda"; ctx.font = "bold 18px sans-serif"; ctx.fillText("🔎 Provas: " + pistasColetadas + " / " + totalPistas, 45, 48);
    ...
    ctx.fillStyle = "white"; ctx.font = "bold 16px sans-serif"; ctx.fillText("📓 Aperte [ C ] para o Caderno", 255, 48);
    ...
    ctx.fillStyle = corTempo; ctx.font = "bold 20px Arial"; ctx.fillText("⏳ Tempo: " + formatarTempo(tempoRestante), canvas.width - 200, 48);

    if (cadernoAberto) {
        ctx.fillStyle = "rgba(0, 0, 0, 0.8)"; ctx.fillRect(0, 0, canvas.width, canvas.height);
        let cX = canvas.width / 2 - 350; let cY = 100;
        ctx.fillStyle = "#fef3c7"; ctx.fillRect(cX, cY, 700, 500); ctx.strokeStyle = "#8b5a2b"; ctx.lineWidth = 6; ctx.strokeRect(cX, cY, 700, 500);
        ctx.fillStyle = "#8b5a2b"; ctx.fillRect(cX, cY, 40, 500);
        for(let i=0; i<10; i++) {
            ctx.fillStyle = "white"; ctx.beginPath(); ctx.arc(cX + 20, cY + 40 + (i*45), 10, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = "rgba(0,0,0,0.5)"; ctx.beginPath(); ctx.arc(cX + 20, cY + 40 + (i*45), 10, 0, Math.PI * 2); ctx.stroke();
        }
    ...
    if (estadoJogo === "DIALOGO" && !cadernoAberto) {
        ...
        if (textoResposta !== "") {
            ...
            if (charIndex >= textoResposta.length) { ctx.fillStyle = "#94a3b8"; ctx.font = "16px Arial"; ctx.fillText("[ Aperte A (ou ESPAÇO) para fechar ]", 190, 700); }
            ...
            ctx.fillText(npcFoco.pergunta1, 210, 665); ctx.fillText(npcFoco.pergunta2, 210, 695);
        }
    }
    ...
    if (estadoJogo === "FIM") {
        ...
        if (suspeitoSelecionado === 0 && pistasColetadas >= totalPistas) { 
            ctx.fillStyle = "#ffe600"; ctx.font = "bold 70px sans-serif"; ctx.fillText("🏆 FINAL VERDADEIRO", canvas.width / 2, canvas.height / 2 - 20);
        } else if (suspeitoSelecionado === 0 && pistasColetadas < totalPistas) {
            ctx.fillStyle = "#a8a8a8"; ctx.font = "bold 70px sans-serif"; ctx.fillText("⚖️ FINAL NEUTRO", canvas.width / 2, canvas.height / 2 - 20);
        } else if (suspeitoSelecionado === 2) {
            ctx.fillStyle = "#b5179e"; ctx.font = "bold 70px sans-serif"; ctx.fillText("🤫 FINAL SECRETO", canvas.width / 2, canvas.height / 2 - 20);
        } else {
            ctx.fillStyle = "#ff4444"; ctx.font = "bold 70px sans-serif"; ctx.fillText("❌ FINAL RUIM", canvas.width / 2, canvas.height / 2 - 20);
        }
